...
// ---- debug tools
const DBG=['Resume','Settings','Debug','Quit to title'];
function dbgPause(sel){const x=105,y=34,w=110,h=118;return dim()+r(x,y,w,h,P.plankDk)+r(x+2,y+2,w-4,h-4,'#6d4527')+t(160,y+16,'Paused',7,P.text,'middle')+DBG.map((l,i)=>plank(x+12,y+28+i*20,w-24,l,i==sel)).join('')+r(x+w-20,y+4,16,7,'#b8462e')+t(x+w-12,y+9.5,'DEV',3.5,P.text,'middle')}
const row=(x,y,w,label,val,sel,h=12)=>r(x,y,w,h,sel?'#fff1c9':P.plankDk)+r(x+1,y+1,w-2,h-2,sel?'#c98a4a':P.plank)+t(x+5,y+h/2+2,label,4.5)+(val!==undefined&&val!==''?t(x+w-5,y+h/2+2,(sel?'◀ ':'')+val+(sel?' ▶':''),4.5,'#ffe2a8','end'):'');
function readout(){return r(236,4,80,34,'#000','opacity=".6"')+t(240,12,'DAY 3 16:30',4,'#9fe89f')+t(240,20,'SPEED x4',4,'#9fe89f')+t(240,28,'FPS 60',4,'#9fe89f')+t(240,36,'X 152 Y 84',4,'#9fe89f')}
function collision(){return r(26,124,14,30,'none','stroke="#ff3b3b" stroke-width="1"')+r(266,128,14,26,'none','stroke="#ff3b3b" stroke-width="1"')+r(0,0,320,40,'none','stroke="#ff3b3b" stroke-width="1"')+`<circle cx="211" cy="102" r="12" fill="none" stroke="#3bd1ff" stroke-width="1"/>`+`<circle cx="103" cy="113" r="12" fill="none" stroke="#3bd1ff" stroke-width="1"/>`+r(152,90,8,6,'none','stroke="#ff3b3b" stroke-width="1"')}
const HTAB=hint([['Q E','Page'],['←→','Change'],['ESC','Back']]);
const HCH=hint([['ENTER','Select'],['←→','Change'],['ESC','Back']]);
// A: boards, like Settings
const SECT=['Time and speed','Items','Place','Story','Survival','Show'];
function boardA(title,rows,sel,w=150){const x=160-w/2,h=30+rows.length*16,y=Math.max(8,90-h/2);let s=dim(.55)+r(x,y,w,h,P.plankDk)+r(x+2,y+2,w-4,h-4,'#6d4527')+t(160,y+15,title,6,P.text,'middle');rows.forEach((q,i)=>s+=Array.isArray(q)?row(x+10,y+24+i*16,w-20,q[0],q[1],i==sel):plank(x+10,y+23+i*16,w-20,q,i==sel,13));return s}
// B: one panel down the right side, pages by Tab, world visible and live
const TABS=['Time','Items','Place','Story','Surv.','Show'];
function panelB(tab,rows,sel){const x=176,w=144;let s=r(x,0,w,164,P.plankDk)+r(x+2,2,w-4,160,'#6d4527')+t(x+w/2,14,'Debug',6,P.text,'middle');
 TABS.forEach((l,i)=>{const tx=x+4+i*23;s+=r(tx,20,22,11,i==tab?'#c98a4a':'#4a2e1a')+t(tx+11,28,l,3.5,P.text,'middle')});
 rows.forEach((q,i)=>s+=row(x+6,36+i*14,w-12,q[0],q[1],i==sel));return s}
