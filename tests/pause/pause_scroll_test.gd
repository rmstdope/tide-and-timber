...
## largest combination there is. Since tr-1o0.1 (640x360) even that rests a 236x128 three-plank board
## that fits, so every scrolling test fails its precondition: a navigator decision, not a stale number.
...
func test_normal_board_does_not_scroll() -> void:
	...
	assert_that(_node("Panel").get_rect()).is_equal(_centred(Vector2(144, 96)))
