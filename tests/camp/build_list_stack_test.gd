...
##
## At 640x360 UI size alone never stacks the list (206 * 2 fits the picture). The stacked layout is
## reached at UI Largest and Text Largest in the default 1280x720 window: the grown side-by-side list
## (350) drawn 2x is wider than Screen.WIDTH, and the stacked list is 262 x 111, rows 246 x 37.
...
func test_normal_is_side_by_side() -> void:
	...
	assert_vector(list.rows[1].position).is_equal(Vector2(8, 32))
...
func test_largest_stacks_every_row() -> void:
	...
	assert_vector(list.rows[0].position).is_equal(Vector2(8, 27))
	assert_vector(list.rows[1].position).is_equal(Vector2(8, 66))
	# 100 + RIGHT_OF_HIM fits (524 wide); 150 - ABOVE_HIM - 222 is above the picture, so held at 2
...
func test_a_size_change_restacks_at_once_highlight_unmoved() -> void:
	...
	assert_bool(list.stacked).is_false()
	# UI Large, words still 2x its scale: the grown side-by-side list (350 * 1.5 fits the picture)
	assert_vector(list.size).is_equal(Vector2(350, 75))
	assert_vector(list.position).is_equal(BuildList.top_left_for(HIM, 1.5, Vector2(350, 75)))
...
func test_one_row_too_wide_stacks_both() -> void:
	...
	list.place_beside(HIM)
	assert_float(list.side_by_side_width()).is_greater(206.0)
	assert_bool(list.stacked).is_true()
